...
namespace FinchUtils.Event
{
    public static class EventDispatcher {
        private static readonly EventRouter Router = new();

        public static void RegEventListener(string eventType, Action handler) {
            Router.RegEventListener(eventType, handler);
        }
        ...
        public static void UnRegEventListener(string eventType, Action handler) {
            Router.UnRegEventListener(eventType, handler);
        }
        ...
        private sealed class EventRouter {
            ...
            private void OnListenerAdding(string eventType, Delegate listenerAdded) {
                ...
                if (eventDelegate != null && eventDelegate.GetType() != listenerAdded.GetType()) {
                    throw new Exception(
                        $"Try to add not correct event {eventType}. Current type is {eventDelegate.GetType().Name}, adding type is {listenerAdded.GetType().Name}");
                }
            }
            ...
            private bool OnListenerRemoving(string eventType, Delegate listenerRemoved) {
                ...
                if (eventDelegate != null && eventDelegate.GetType() != listenerRemoved.GetType()) {
                    throw new Exception(
                        $"Remove listener {eventType} failed, Current type is {eventDelegate.GetType()}, adding type is {listenerRemoved.GetType()}.");
                }
            ...
            public void RegEventListener<T1, T2, T3>(string eventType, Action<T1, T2, T3> handler) {
                OnListenerAdding(eventType, handler);
                _eventRouter[eventType] =
                    (Action<T1, T2, T3>)Delegate.Combine((Action<T1, T2, T3>)_eventRouter[eventType], handler);
            }
            ...
            public void UnRegEventListener<T1, T2, T3>(string eventType, Action<T1, T2, T3> handler) {
                ...
                _eventRouter[eventType] =
                    (Action<T1, T2, T3>)Delegate.Remove((Action<T1, T2, T3>)_eventRouter[eventType], handler);
                OnListenerRemoved(eventType);
            }
